...
conn = sqlite3.connect('cdc.sqlite')
cur = conn.cursor()
...
for row in cur :
    nodes.append(row)
    rank = row[2]
    if maxrank < rank or maxrank is 0 : maxrank = rank
    if minrank > rank or minrank is 0 : minrank = rank
    if len(nodes) > howmany : break

if maxrank == minrank or maxrank is 0 or minrank is 0:
